...
class resolvedMorseLink: #data is a dictionary where the entries are of the form (n,((i,j), move))
    def __init__(self, data):
        ...
        for n in crossingToComponentsDict.keys():
            ...
            #This dictionary takes a crossing an outputs the lowest cup
...
def Sq1(khovanovGenerators, gradings, kernel):
    ...
    for i in nMatrix.rows:
        ...
        #print('Sq1:', gradings, kernel, khovanovGenerators.hqToSparseMatrix[(gradings[0]+2,gradings[1])]*output)
    return output
...
def zCoefficient(flowCategory, matchings, z):
    ...
    for K in cycleSet:
        ...
        for i in range(cLength): #Adds the products of adjacencies
            ...
            KElement += max(triple[0],triple[2])#Adds the max of a and c
            if triple[1] < triple[0] and triple[1] < triple[2]:
                KElement +=  1   
            if triple[0] != triple[2] and sorted(triple)[1] == triple[0]:
                KElement += 1
            if triple[0] == triple[2]:
                middleBoundaryArc = K[(i+1)%cLength]
                if (middleBoundaryArc[0][0], middleBoundaryArc[1][0]) not in matchings.boundaryMatchings[middleBoundaryArc[0][1]] and triple[1]>triple[0]:
                    #print(i, 'reversed')
                    KElement += 1
                elif (middleBoundaryArc[0][0], middleBoundaryArc[1][0]) in matchings.boundaryMatchings[middleBoundaryArc[0][1]] and triple[1]<triple[0]:
                    KElement += 1
        KElement += 1
        Z2Element += KElement
    return Z2Element%2

def Sq2(gradings, flowCategory, khovanovGenerators, kernel):
    ...
    if khovanovGenerators.hqToSparseMatrix[(gradings[0]+2,gradings[1])]*Sq2Set != set():
        print('kernelTest:', khovanovGenerators.hqToSparseMatrix[(gradings[0]+2,gradings[1])]*Sq2Set)
...
def Sq2Matrix(khovanovGenerators, flowCategory, gradings):
    ...
    for j in range(nHomologyDim):
        inputElement = nHomology.orderedSparseEchelonSet[j]
        outputElement = Sq2(gradings, flowCategory, khovanovGenerators, inputElement)
...
knot9_42Dict = (((1, 2), 0), ((8, 7), 0), ((2, 7), "+"), ((6, 5), 0), ((2, 5), "+"), ((6, 7), "+"), ((6, 7), "+"), ((7, 8), 1), ((4, 3), 0), ((2, 3), "-"), ((4, 5), "-"), ((4, 5), "-"), ((5, 6), 1), ((2, 3), "-"), ((3, 4), "+"), ((2, 3), 1), ((4, 1), 1))
...
disjointMirrorTrefoilDict = (((1, 2), 0), ((4, 3), 0), ((2, 3), "-"), ((2, 3), "-"), ((2, 3), "-"), ((2, 1), 1), ((3, 4), 1), ((1, 2), 0), ((4, 3), 0), ((2, 3), "-"), ((2, 3), "-"), ((2, 3), "-"), ((2, 1), 1), ((3, 4), 1))
